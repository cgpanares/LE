...
def create_Run_Template(temp_list_statement, created_script, prefix, outputloc, hostname, ssm_comment):
		print("Chosen Option: Create and run the template", flush=True)
...
def findTemplateReport(name_of_script):
		report_location = "C:\\Users\\" + str(os.environ['USERNAME']) + "\\Documents\\ExecuteSSMReport"
		time.sleep(1)
		csv_file = find(name_of_script, report_location)
		return csv_file
...
def checkCredentials(cred_to_check):
